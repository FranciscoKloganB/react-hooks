...
In this module I learned how to make HTTP requests using the useEffect. It was trivial,
however, I also learned how to handle errors using status codes instead of booleans.
While it seems kind of obvious to use this aproach after trying it, it is not an
immediate approach when you used to simple boolean logic.

I learned how to make a custom ErrorBoundary, but it is kind of pointless to do one. So
instead we use `react-error-boundary` which offers this functionality with very nice
features out of the box.

We use a this `ErrorBoundary`'s props `resetKeys` and callback hooks `onError`,
for two reasons:

- The `resetKeys` emulates the native React `key` prop on `ErrorBoundary`, but
accepts an array, allowing us to navigate away from an error state with new
queries and button clicks that do not explicitly reset the state that caused the
error. E.g., user is in error state and types a valid pokemon state. This approach
does not cause remounting like the native `key` approach.
- The `onError` callback, allow us to exit the error state explicitly without
remounting the entire `ErrorBoundary` and its children. E.g., clicking a button
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=francisco.teixeira.de.barros%40outlook.com).
