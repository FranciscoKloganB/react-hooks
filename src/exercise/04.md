...
We can decrease complexity of our components by deriving state from other
pieces of data, this avoids unncessary re-renders. In vue, this would be
equivalent to data properties and computed properties, which are just values
which are calculated based on essential state data.

Furthermore, from the first lesson, we learned that we should not mutate state
directly. That is why, first, we copy the current state value and only after
mutating that copy do we 'commit' the changes using the `setSquares` state handler.

Once again, we remind ourselves that localStorage always save data as a string,
consequently we need to serialize and deserialize any data we want to store in there
unless it is ofc, a string. We did this using JSON stringify and parse methods. We
also applied, again, optimizations to only run the `useEffect` when the dependency
`squares` is updated and also initialize state with an anonymous function to avoid
parsing from localStorage whenever squares is updated.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=04%3A%20useState%3A%20tic%20tac%20toe&em=francisco.teixeira.de.barros%40outlook.com).
