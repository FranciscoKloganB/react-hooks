...
In this lesson I learned how to use React useState and useEffect in order to 
persist data accross browser refreshes, leveraging local storage.

However, since reading and writting to local storage is expensive we use 
anonymous (arrow) functions to ensure that useState initial value is only set
and read from local storage once. Denoting that, this should only be done
if and only if, we need to perform expensive computations when setting the
initial state; If we are just passing a value by reference to the useState
it is faster for the re-render component process to read that value than it is
to read an anonymous function (even if it's body is lost after first execution).
To avoid writting the value of state to local storage on every re-render, unless
that re-render was caused by a change in the state's value and not in the parents,
we use the `useEffects` dependencies parameter. Like this, `useEffect` only runs
the body when one of the declared dependencies caused the re-render. Once again,
this is only worth it, if the `useEffect` job is computationally expensive. 
Additionally, the dependencies array, when declared should not be empty, 
otherwise `useEffect` never runs and, when used it should only contain pointers
that work well with shallow comparison. Complex expressions such has `dictionaries`
or `hashmaps` do not work well with this array, altough further optimizations
can be employed to go around this.

I also learned how to make custom, reuseable hooks to avoid code repetition. By
convention, custom hooks should start with the `use` prefix.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=francisco.teixeira.de.barros%40outlook.com).
